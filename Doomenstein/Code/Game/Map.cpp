...
void Map::AddGeometryForWall(const AABB3& bounds, const AABB2& UV) {
	...
	mins.z += 1.f;
	maxs.z += 1.f; 
	x_min_y_min_z_max=Vec3(mins.x, mins.y, maxs.z);
	x_min_y_max_z_min=Vec3(mins.x, maxs.y, mins.z);
	x_min_y_max_z_max=Vec3(mins.x, maxs.y, maxs.z);
	x_max_y_min_z_min=Vec3(maxs.x, mins.y, mins.z);
	x_max_y_min_z_max=Vec3(maxs.x, mins.y, maxs.z);
	x_max_y_max_z_min=Vec3(maxs.x, maxs.y, mins.z);
...
void Map::Update(float deltaSeconds) {
	if (GetActorByHandle(*DemonAIController::g_party[0])==nullptr) {
		g_theGame->ExitState(g_theGame->m_currentState);
		g_theGame->EnterState(GameState::GAMEOVER);
		return;
	}
		...
		*/
		...
		for (int i = 0; i < m_actors.size(); i++) {
			...
			if (actor && actor->m_handle->IsValid()) {
				float distToPlayer = (actor->m_bottomPosition - g_player1Controller->m_position).GetLengthSquared();
				
				if (distToPlayer <= 400.f) {
					
					//actor->UpdatePhysics(deltaSeconds);
					
					if (actor->m_AIController && actor->m_controller != g_player1Controller) {
						actor->m_AIController->Update();
					}
					if (actor->m_MarineAI && actor->m_controller != g_player1Controller) {
		...
		while (m_physicsTimer->DecrementPeriodIfElapsed())
		{
			for (int i = 0; i < m_actors.size(); i++) {
				...
				if (actor && actor->m_handle->IsValid()) {
					...
					if (distToPlayer <= 400.f) {
						actor->UpdatePhysics(PHYSICS_DELTA_SECOND);
					}
				}
			}
			CollideAllActors();
		...
		for (int i = 0; i < m_actors.size(); i++) {
			if (m_actors[i] != nullptr) {
				if (m_actors[i]->m_definition->m_base->m_name.compare("Marine") == 0) {
					marine++;
				}
			}
		}
		if (marine == 0 && GetActorByHandle(*DemonAIController::g_party[0])&& !GetActorByHandle(*DemonAIController::g_party[0])->m_dead) {
			g_theGame->ExitState(g_theGame->m_currentState);
...
/*
    ...
    std::string marineInfo = "Marine left: " + std::to_string(marine);
	if (marine == 0) {
		marineInfo = "Victory!";
	}
	DebugAddScreenText(marineInfo, AABB2(Vec2(0.5f, 23.f), Vec2(46.f, 24.f)), .3f, Vec2(1.f, 0.f), 0.f);*/
}
			...
			if (IsPointInsideDirectedSector2D(Vec2(actorPos.x, actorPos.y), Vec2(pos.x, pos.y), Vec2(fwd.x, fwd.y), cone, radius)) {
				if (actor->m_definition->m_base->m_faction != GetActorByHandle(*owner)->m_definition->m_base->m_faction) {
					...
					if (actor->m_AIController && actor->m_controller == actor->m_AIController) {
						...
						if (actor->m_controller == g_player1Controller) {
							DemonAIController::GlobalAggro(owner);
						}
					}
					if (actor->m_MarineAI && actor->m_controller == actor->m_MarineAI) {
						actor->m_MarineAI->DamagedBy(owner);
						if (GetActorByHandle(*owner)->m_controller == g_player1Controller) {
						    DemonAIController::GlobalAggro(actor->m_handle);
						}
					}
				}
			}
	...
	if (finalActorResult.m_didImpact&& finalActorResult.m_impactPos==worldResult.m_impactPos) {
		if (true/*resultActor->m_definition->m_base->m_faction != projectileOwner->m_definition->m_base->m_faction*/) {
			...
			if (resultActor->m_AIController && resultActor->m_controller == resultActor->m_AIController) {
				...
				if (resultActor->m_controller == g_player1Controller) {
					DemonAIController::GlobalAggro(owner);
				}
			}
...
void Map::SpawnPlayer() {
	if ((int)m_actors.size() < ActorHandle::MAX_ACTOR_INDEX && m_nextActorUID < ActorHandle::MAX_ACTOR_UID) {
		if (!m_player1Handle || !GetActorByHandle(*m_player1Handle))
		{
			...
			actor->m_bottomPosition = Vec3(24.f, 2.f, 0.f);
			actor->m_orientation = EulerAngles(90.f, 0.f, 0.f);
