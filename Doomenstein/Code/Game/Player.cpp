...
void PlayerController::UpdateInput(float deltaSeconds) {
	...
	if (g_theInput->GetCursorMode()==CursorMode::FPS) {
		...
		if (m_mode == CameraMode::POSSESS) {
			...
			finalValX = fminf(finalValX, 270.f);
		}
	...
	if (g_theInput->WasKeyJustPressed('Q')) {
		m_thirdPersonDist -= 2.f*deltaSeconds;
		...
		m_thirdPersonDist += 2.f*deltaSeconds;
		if (m_thirdPersonDist > 2.f) {
			m_thirdPersonDist = 2.f;
		}
	}
	if (m_mode == CameraMode::POSSESS) {
		Actor* possess = m_map->GetActorByHandle(*m_ownerHandle);
	...
	if (m_map->GetActorByHandle(*m_ownerHandle) != nullptr) {
		...
		if (g_theInput->WasKeyJustPressed(KEYCODE_LEFT_MOUSE)) {
			if (possess->m_weapons[possess->m_currentWeaponIndex]->m_definition->m_name.compare("Meat") == 0&&!g_theInput->WasKeyJustReleased(KEYCODE_LEFT_MOUSE)) {
				...
				possess->TryChangeAction("Attack");
			}
...
void PlayerController::UpdateCamera() {
	...
	if (m_mode == CameraMode::POSSESS) {
		if (m_ownerHandle&&m_map->GetActorByHandle(*m_ownerHandle) != nullptr) {
				...
				m_position += Vec3::MakeFromPolarDegree(m_orientation.m_yawDegrees, 30.f, -m_thirdPersonDist);;
...
void PlayerController::RenderStats(Weapon* weapon) {
	...
	AABB2 KillSquare = AABB2(Vec2(0.f * HUDSizeX, boxLow), Vec2(.15f * HUDSizeX, .45f * boxHight));
	AABB2 DeathSquare = AABB2(Vec2(0.f * HUDSizeX, boxLow), Vec2(1.87f * HUDSizeX, .45f * boxHight));
	...
	if (owner) {
		...
		std::vector<Vertex_PCU> verts;
		g_squirrelFont->AddVertsForTextInBox2D(verts, health,HealthSquare, cellHeight);
		g_squirrelFont->AddVertsForTextInBox2D(verts, std::to_string(m_ammo), AmmoSquare, cellHeight);
		//Render kill needed
		int marine = 0;
		for (int i = 0; i < m_map->m_actors.size(); i++) {
			if (m_map->m_actors[i] != nullptr) {
				if (m_map->m_actors[i]->m_definition->m_base->m_name.compare("Marine") == 0) {
					marine++;
				}
			}
		}
		std::string marineInfo = std::to_string(marine);
		g_squirrelFont->AddVertsForTextInBox2D(verts, marineInfo, KillSquare, cellHeight);
		g_squirrelFont->AddVertsForTextInBox2D(verts, std::to_string(DemonAIController::g_party.size()), DeathSquare, cellHeight);
		g_theRenderer->DrawVertexArray(verts);
		verts.clear();
		//render weapon
		if (m_map->m_gold) {
