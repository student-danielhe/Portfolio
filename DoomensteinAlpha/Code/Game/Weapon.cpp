	...
	if (m_timer->HasPeriodElapsed()) {
		...
		if (owner != nullptr) {
			...
			for (int i = 0; i < m_definition->m_projectileCount; i++) {
				...
				else if (owner->m_controller == g_player2Controller) {
					fwd = g_player2Controller->m_orientation;
				}
				else {
			...
			for (int i = 0; i < m_definition->m_rayCount; i++) {
				...
				if (owner->m_controller == g_player2Controller) {
					fwd = g_player2Controller->GetFwdVector();
				}
				else {
