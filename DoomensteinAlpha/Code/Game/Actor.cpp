...
void  Actor::UpdatePhysics(float deltaSeconds) {
	...
	if (m_definition->m_physics->m_simulated&&!m_dead) {
		...
		AddForce(m_definition->m_physics->m_drag * -m_velocity.GetNormalized());
		if (!m_dead) {
		...
		if (m_definition->m_base->m_name.compare("Meat") == 0&&m_owner!=other.m_handle) {
			if (other.m_definition->m_base->m_faction == Faction_Actor::DEMON) {
				...
				if (g_player1Controller->m_ownerHandle==other.m_handle) {
					...
					SoundID audio = g_theAudio->CreateOrGetSound("Data/Gold/Fresh Meat.mp3");
					g_theAudio->StartSound(audio);
					m_dead = true;
				}
				else {
...
void Actor::RenderHealthBar(PlayerController* player) {
	if (!m_dead) {
		...
		AddVertsForQuad3D(verts, Vec3(0.f, HPBounds.m_mins.x, HPBounds.m_mins.y), 
			                     Vec3(0.f, HPBounds.m_maxs.x * percentile, HPBounds.m_mins.y),
			                     Vec3(0.f, HPBounds.m_maxs.x * percentile, HPBounds.m_maxs.y),
			                     Vec3(0.f, HPBounds.m_mins.x, HPBounds.m_maxs.y), Rgba8::GREEN, AABB2::DEFAULT);
		g_theRenderer->DrawVertexArray(verts);
		...
		AddVertsForQuad3D(verts, Vec3(0.f, HPBounds.m_maxs.x*percentile, HPBounds.m_mins.y),
			Vec3(0.f, HPBounds.m_maxs.x, HPBounds.m_mins.y),
			Vec3(0.f, HPBounds.m_maxs.x, HPBounds.m_maxs.y),
			Vec3(0.f, HPBounds.m_maxs.x*percentile, HPBounds.m_maxs.y), Rgba8::RED, AABB2::DEFAULT);
		g_theRenderer->DrawVertexArray(verts);
	}
		...
		if (!enemy->m_dead) {
			if (OpposingFaction(other)) {
				if (enemy->m_definition->m_base->m_visible) {
					Vec3 targetToMe = m_bottomPosition - enemy->m_bottomPosition;
					...
					if (IsPointInsideDirectedSector2D(enemy->m_bottomPosition,m_bottomPosition,fwd,m_definition->m_ai->m_sightAngle,m_definition->m_ai->m_sightRadius)) {
						RaycastResult3D res = m_map->RaycastAll(m_bottomPosition, targetToMe.GetNormalized(), m_definition->m_ai->m_sightRadius);
						RaycastResult3D actorRes = m_map->RaycastWorldActors(m_bottomPosition, targetToMe.GetNormalized(), m_definition->m_ai->m_sightRadius);
	...
	for (AnimationGroupDefinition* animDef : m_animGroups) {
		if (animDef->m_name.compare(m_currentAction) == 0) {
			if (animDef->scaleBySpeed) {
				m_animClock->SetTimeScale(m_velocity.GetLength() / m_definition->m_physics->m_runSpeed);
			}
			else {
