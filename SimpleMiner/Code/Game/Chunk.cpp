...
void Chunk::CreateBlocks()
{
	...
	continentalMapXY.resize(CHUNK_SIZE_X*CHUNK_SIZE_Y);
	std::vector<float>   erosionMapXY;
	erosionMapXY.resize(CHUNK_SIZE_X*CHUNK_SIZE_Y);
	std::vector<float>   PVMapXY;
	PVMapXY.resize(CHUNK_SIZE_X * CHUNK_SIZE_Y);
	std::vector<float> humidityMapXY;
	humidityMapXY.resize(CHUNK_SIZE_X*CHUNK_SIZE_Y);
	std::vector<float> temperatureMapXY;
	temperatureMapXY.resize(CHUNK_SIZE_X*CHUNK_SIZE_Y);
	std::vector<float> treeMapXY;
	treeMapXY.resize(CHUNK_SIZE_X * CHUNK_SIZE_Y);
			...
			for (int y = 0; y < CHUNK_SIZE_Y; y++) {
				for (int x = 0; x < CHUNK_SIZE_X; x++) {
					...
					if (def != BlockDefinition::g_blockDefinitions[BLOCK_TYPE_AIR]) {
						...
						if (!BlockOpaque(Vec3((float)x, (float)y, (float)(z + 1)))) {
							...
							Rgba8 color = Rgba8((unsigned char)b.GetOutdoor(), (unsigned char)b.GetIndoor(), (unsigned char)255);



							color = GetWorldColor(color);
						...
						if (!BlockOpaque(Vec3((float)x, (float)(y - 1), (float)z))) {
							//Front
							indexStart = (int)m_verts.size();
							m_indexes.push_back(indexStart);
							m_indexes.push_back(indexStart + 2);
						...
						if (!BlockOpaque(Vec3((float)(x - 1), (float)y, (float)z))) {
							//Left
							indexStart = (int)m_verts.size();
							m_indexes.push_back(indexStart);
							m_indexes.push_back(indexStart + 2);
...
{
	return GetBlockAtPos(IntVec3((int)pos.x, (int)pos.y, (int)pos.z)).m_type!=BLOCK_TYPE_AIR;
}
...
Rgba8 Chunk::GetWorldColor(Rgba8 input)
{
	...
	return Rgba8((unsigned char)roundf(finalColor.x*255.f), (unsigned char)roundf(finalColor.y*255.f), (unsigned char)roundf(finalColor.z*255.f),255);
...
int Chunk::FindFirstSolidBlockBelow(IntVec3 coord)
{
	...
	for (int z = coord.z; z >= 0; z--) {
		...
		if (block.m_type != BLOCK_TYPE_AIR) {
			return z;
		}
	}
	return 0;
