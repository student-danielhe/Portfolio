...
void Map::Update(float deltaSeconds)
{
	...
	for (auto pair: m_activeChunks) {
	    if(pair.second->m_chunkState==ChunkState::ACTIVE)
...
void Map::HandleKeyPressed()
{
	...
	if (g_theInput->WasKeyJustReleased('1')) {
		m_placeingType=BLOCK_TYPE_GLOWSTONE;
	}
	if (g_theInput->WasKeyJustReleased('2')) {
		m_placeingType = BLOCK_TYPE_COBBLESTONE;
	}
	if (g_theInput->WasKeyJustReleased('3')) {
		m_placeingType = BLOCK_TYPE_CHISELEDBRICK;
	}
